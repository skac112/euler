...
/**
  * Basic implementation of mutable graph optimised for quick access to nodes by index and
  * for quick access to edges of a given node.
...
//  import skac.euler.General._
//  type G = Graph[ND, ED]
//  override type NodesT = scala.collection.mutable.ArrayBuffer[NodeStruct[ND, ED]]
...
//
//  override def addNode[SND >: ND](Data: SND): Graph[ND, ED] = {
//    Nodes.append(NodeStruct(newNodeInfo(Data), Map[Any, EdgeInfo[ED]](), Map[Any, EdgeInfo[ED]]()))
//    this
//  }
//
//  override def addEdge(Data: ED, SrcNode: NodeDesignator, DstNode: NodeDesignator) = {
...
//    edges(NodeDes) foreach {ei: EdgeInfo[_] => removeEdge(ei.ID.eid)}
//    NodeDes match {
...
//      case NodeDataDesignator(data) => Nodes --= (nodesOf(data.asInstanceOf[ND]) map {node => {Nodes find {_.nodeInfo == node} get}})
